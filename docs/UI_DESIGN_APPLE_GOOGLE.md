...
| `--border` / `--border-subtle`                                  | 分隔线与输入框描边；弱化默认「硬线」，聚焦时用主色描边。                            |
...
1. **搜索框**：更高胶囊圆角（`border-radius: 28px`）、聚焦环 `box-shadow` + 细描边；图标与内边距按 44px 视觉高度对齐；快捷键徽标使用表面色与小圆角 pill。
...
4. **结果卡片**：略增大圆角与内边距；阴影使用双层 elevation；hover 时 `translateY(-1px)` 与阴影加深（在 `prefers-reduced-motion: reduce` 下关闭动画与过渡）。
5. **分页**：圆形页码保留，增强 `focus-visible` 环与 hover 状态。
6. **无障碍与交互**：所有 `button`、链接型控件补充 `:focus-visible` 轮廓；`prefers-reduced-motion: reduce` 全局缩短动画/过渡；`html` 在允许动效时使用平滑滚动（减少模式下关闭）。
